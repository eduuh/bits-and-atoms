...
summary: >-
  A tour of my Kubernetes homelab on Talos Linux with GitOps and full
  observability, and why I tore the whole thing down in August 2026.
tags:
  - Self-Hosted
published: true
pinned: true
...
<Callout type="warning" title="Update, August 2026: this cluster no longer exists">

Everything below describes a Kubernetes homelab that I have since torn down. It is
left up as a record, not a recommendation.

Two of the three nodes are gone. I rebuilt the AMD mini PC as a desktop, and the
ACEMAGIC died. That left one machine, and a single-node Kubernetes cluster is a lot
of moving parts to justify. Talos, Flux, Longhorn, MetalLB and the rest came down
with it.

What replaced it is smaller and, honestly, more useful day to day: one Glovary
running Proxmox, with OPNsense as a VM doing segmented routing for the house. The
interesting problems moved from orchestration to networking.

That setup is written up in [My Home Lab, V2](/blog/my-home-lab-v2). This post stays
as it was.
...
Instead of a traditional Linux distribution, I run [Talos Linux](https://www.talos.dev/), an immutable, API-managed OS designed for Kubernetes.
...
My rule for the homelab is simple: **I care about getting data back, not about staying up.** Services can fall over. Data can't disappear. [Longhorn](https://longhorn.io/) fits that. Distributed block storage across the cluster, with 2-3 replicas of each volume spread over nodes, automated snapshots, and both ReadWriteOnce and ReadWriteMany modes.
