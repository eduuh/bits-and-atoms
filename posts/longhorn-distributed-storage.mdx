...
Scaling is zero-config: add a new node, configure its Longhorn `Node` resource, and replicas start landing there automatically. Online volume expansion just works. Bump the size in the manifest, Flux applies it, and Longhorn resizes without a pod restart for most workloads. And it pairs well with Talos. The immutable OS model fits Longhorn's declarative approach. Disk management is config, not commands.
